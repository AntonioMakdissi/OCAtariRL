...
class PositionHistoryEnv(gym.Env):
    ...
    def __init__(self, env_name : str, render_mode : str=None) -> None:
        ...
        self.current_vector = np.zeros(6 * len(self.reference_list), dtype=np.float32)#change this to -1


    @property
    def observation_space(self):
        vl = len(self.reference_list) * 6
    ...
    def _obj2vec(self):
        ...
        for o in self.ocatari_env.objects: # populate out_vector with object instance
            ...
            #NOTE
            #def h_coords(self): return [self._xy, self.prev_xy] if object just appeared then xy=prev_xy
            # Calculate speed
            if len(o.h_coords) == 2: # Check if there are previous coordinates
                ...
                speed_x = (curr_x - prev_x)#/4 #divid by 4 for speed
                speed_y = (curr_y - prev_y)#/4  #divide by 4 for speed
            else:
                speed_x = 0
                speed_y = 0
...
if __name__ == "__main__":
    cores = 1
    exp_name = "ALE/nomralize_FALSE_missingOb_ZERO_speeds_TRUE"
    env_str = "ALE/Seaquest-v5"
    env = SubprocVecEnv([make_env(game=env_str,save=exp_name, rank=i, seed=42) for i in range(cores)])
    #eval_env = SubprocVecEnv([make_env(game=env_str,save=exp_name, rank=i, seed=42, is_eval=True) for i in range(cores)])

    # eval_callback = EvalCallback(
    #     eval_env,
    #     n_eval_episodes=4,
    #     best_model_save_path=exp_name + "/",
    #     log_path=exp_name + "/",
    #     eval_freq=max(2_000_000 // cores, 1),
    #     deterministic=True,
    #     render=False)
    ...
    clipping_eps = 0.1
    model = PPO(
        "MlpPolicy",#CNNPolicy for comparision
        n_steps=128,
        ...
        env=env,
        verbose=1)
    #model.learn(1e4)#was 1e4 for testing
...
print("done")
#make sure of the time consisstency of the vector-> c bon
#and find out the values meaning->xy and prev_xy
#try where missing objects are 0 anotherr where they are -1
#want to calcualte velocities of all objects,(/4 frames as we skip by default 4 frames)
#remove W and h from the vector if they are presented
#see if we learn better with or without the normalization
#svaing the result